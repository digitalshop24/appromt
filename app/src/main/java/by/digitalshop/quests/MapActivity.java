...
public class MapActivity extends BaseActivity implements
    ...
    private static final int PROXIMITY_THRESHOLD = 1000;
    private static final int NOTIFICATION_THRESHOLD = 1000;
    private static final int ID_PROXIMITY_NOTIFICATION = 123;
...
//        mMapView.setOnTouchListener(this);
    ...
    private void showSliderMarkers() {
        sliderBackground.setVisibility(View.VISIBLE);
        final ViewPager pager = containerSlider.getViewPager();
        final MarkerPagerAdapter adapter = new MarkerPagerAdapter(this, DsqApplication.sDaoSession.getMapQuestMarkerDao().loadAll());
        ...
        pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                MapQuestMarker marker = adapter.getList().get(position);
        ...
        GeoPoint point = new GeoPoint(adapter.getList().get(0).getLatitude(), adapter.getList().get(0).getLongitude());
...
//                DsqApplication.sDaoSession.getMapQuestMarkerDao().update(marker);
                AsyncSession session = DsqApplication.sDaoSession.startAsyncSession();
                session.setListener(new AsyncOperationListener() {
                    @Override
                    public void onAsyncOperationCompleted(AsyncOperation operation) {
    ...
    public void onLocationChanged(Location location) {
        ...
        for (MapQuestMarker marker : markers) {
            ...
            if (distance < NOTIFICATION_THRESHOLD) {
                ...
                notificationMessage.append(" ");
                notificationMessage.append(distance);
        ...
        if (notificationMessage.length() > 1) {
            ...
            Notification notification = Notifications.createNotification(this, getString(R.string.notification_title), notificationMessage.toString());
    ...
    private class MyOverlay extends Overlay {
        ...
        public boolean onSingleTapUp(float v, float v1) {
            ...
            if (item != null) {
                ...
                showInfoWindow((int) item.getScreenPoint().getX(), (int) item.getScreenPoint().getY(), overlayItem.title, overlayItem.text, overlayItem.id);
            }
